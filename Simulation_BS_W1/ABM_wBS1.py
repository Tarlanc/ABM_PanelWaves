...
pset = {'coeff_a':(.170,.08,'norm'), ##Effect of Media Reliance
        'coeff_b':(-.0922,.05,'norm'), ##Intercept: Media Impact
        'coeff_c':(.0702,.035,'norm'), ##Intercept: Social Impact
        'coeff_d':(2.03,.2,'normp'), ##Ratio of Spatial vs. Ideological distance
        'coeff_e':(-.041,.02,'norm')} ## Effect of Attitude certainty
...
outfile1 = "SimResult_BS_full.txt"        ## Complete collection of simulation results for each parameter set
outfile2 = "SimResult_BS.txt"             ## Summary per generation, providing M, SD and 95% CI for each parameter
summary_file = 'Summary_BS.txt'           ## Summary after maximal number of generations, including cross-validation with test data

res_outfile = 0 ## Setting to reset or retain outfile1 and 2: 1: New Simulation output, 0: Append to previous output
res_pset = 1 ## Setting to reset the prior distribution after each bootstrapping sample: 1: reset / 0: use the posterior from last sample
