...
outfile1 = "SimResult_Bootstrap.txt"
outfile2 = "SimResult_Condensed.txt"
res_outfile = 1 ##1: New Simulation output, 0: Append to previous output


summary_file = 'Summary_Bootstrap.txt'
for bootstep in range(0,g_settings['N_Bootstrap']):
    ##Bootstrapping loop
    write_sample('Survey_Data.dat','bs_sample.dat',g_settings['S_Bootstrap'])
    pset = {'coeff_a':(.1786,.001,'norm'), ##Set prior distributions (m and sd) for each parameter
            'coeff_b':(-.0916,.001,'norm'),
            'coeff_c':(.07392,.0001,'norm'),
            'coeff_d':(1.938,.001,'norm'),
            'coeff_e':(-.0471,.0001,'norm')}
    ...
    prior = evol_initialize(pset,g_settings['Gen_Size']) ## Initialize the the parameters
    ...
    while generation < g_settings['Max_Gen']:
        ...
        for i in range(g_settings['Gen_Size']):
            try:
                param = {}
                for p in pset.keys():
                    param[p] = prior[p]['Curr'][i]

                dics = initialize('bs_sample.dat','Content_Data.dat',verbose=0) ## Initialize the simulation
        ...
        resline = time.ctime()+'\t'+str(pcorr_init_id)+'\t'+str(c_mittel(prior['Results']))+'\t'+str(c_sdev(prior['Results']))
    ...
    if bootstep == 0:
        ...
        fullresult.write('TS\tInitial_Corr\tFinal_Corr\tSD_Final_Corr')
        for p in sorted(prior.keys()):
            fullresult.write('\tM_'+p+'\tSD_'+p)
        fullresult.write('\n')
        fullresult.close()
