...
dta2 = read.table("SimResult_Crossval_full.txt",header=T,sep="\t",quote="")
dta = read.table("SimResult_Crossval.txt",header=T,sep="\t",quote="")
...
# plot3d(av3[,1][start:end][av1[,2]>0.832213],av4[,1][start:end][av1[,2]>0.832213],av1[,2][start:end][av1[,2]>0.832213],
#        xlab=avnames[3],ylab=avnames[4],zlab="Result")

# plot3d(av2[,1][start:end][av1[,2]>0.832213],av3[,1][start:end][av1[,2]>0.832213],av1[,2][start:end][av1[,2]>0.832213],
#        xlab=avnames[2],ylab=avnames[3],zlab="Result")
# 
# plot3d(av5[,1][start:end][av1[,2]>0.832213],av3[,1][start:end][av1[,2]>0.832213],av1[,2][start:end][av1[,2]>0.832213],
#        xlab=avnames[5],ylab=avnames[3],zlab="Result")


v1 = cbind(1/dta2$M_Result,1/dta2$SD_Result,1/dta2$Hig_Result,1/dta2$Low_Result)
v1 = cbind(log(dta2$SD_Result,base=10),log(dta2$SD_Result,base=10),log(dta2$SD_Result,base=10),log(dta2$SD_Result,base=10))
...
for(i in c(3,4,5,2,6,1)){
  ...
  lfdn = 1:length(m)
  plot(lfdn,m,type="l",ylim=c(min.val,max.val),xlim=c(minx,maxx),main=vnames[i],ylab="",xlab="Generation")
