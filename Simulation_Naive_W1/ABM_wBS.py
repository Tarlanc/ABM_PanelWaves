...
pset = {'coeff_a':(0,.01,'norm'), ##Effect of Media Reliance
        ...
        'coeff_d':(1.0,.02,'normp'), ##Ratio of Spatial vs. Ideological distance
...
outfile1 = "SimResult_Crossval.txt"
outfile2 = "SimResult_Crossval_full.txt"
res_outfile = 1 ##1: New Simulation output, 0: Append to previous output
summary_file = 'Summary_Crossval.txt'
    ...
    while generation < g_settings['Max_Gen']:
        ...
        for i in range(g_settings['Gen_Size']): ## Repeat the simulation for each parameter set
            ...
            try:
                ...
                dics = initialize('train_sample.dat','Content_Data.dat',verbose=0) ## Initialize the simulation
...
##            dr2 = (simresult[1]**2-pcorr_init_r**2)*100
    ...
    dics = initialize('test_sample.dat','Content_Data.dat',verbose=0)
    pcorr_init_id = calc_identity(dics[0],var_e1,var_e2)
    simresult = simulate(dics[0],dics[1],simdates,param)
